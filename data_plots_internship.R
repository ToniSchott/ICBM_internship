...
### growth rates ####
#import dataset:
data_r<-read.csv("growthrate_cellcounts.txt")#growthrates
data_ID<-read_excel("Rawdata_Staurastrum.xlsx","ID")
#look at the data:
#hist(log(data_r$r))
...
rm(list=ls())
#import data:
data<-read.csv("median_cellvolume.txt")
...
rm(list=ls())
#import data:
data<-read.csv("staurastrum_cellcount.txt")
